#!/usr/bin/env python
"""Generate one lightweight-FL-NILM (federated mutual learning) training notebook per
client scenario.

The notebook body is assembled from papers/nilm-li-2024/proj/*.py, which are the same
...
path; never hand-edit the .ipynb.
    ...
    python scripts/gen_notebook.py --owner X --clients 100 --session 2 --require-resume \\
        --kernel-source X/nilm-fl-veremi-100-clients --max-hours 11
"""
import argparse, json, re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PROJ = ROOT / "papers/nilm-li-2024/proj"
RUNTIME = json.loads((ROOT / "knowledge/runtime.json").read_text())
META = json.loads((ROOT / "knowledge/meta.json").read_text())
MODULES = ("model", "ckpt", "metrics", "data", "nilm", "evaluate", "driver", "verify")

# batch from knowledge/dataset.md §4 and the owner's instruction of 2026-09-15. Every
# client trains in every round (Algorithm 1 iterates over all households), so the mean
# over clients is always a mean over models trained this round.
...
# Owner's decisions of 2026-09-15 (CONTEXT.md §1): AdamW / wd 1e-4 as in
# knowledge/architecture.md, 50 rounds x 1 local epoch, per-round cosine LR 1e-3 -> 1e-5
# (proj/nilm.py::lr_at), the single LR policy shared by the sibling rebuilds. The paper
# names neither optimizer nor rate for the federated stage.
PAPER = dict(lr=1e-3, lr_schedule="cosine", lr_min=1e-5, weight_decay=1e-4,
             rounds=50, local_epochs=1)
...
# Train: one client's mutual-learning epoch (two forwards + one backward per step) eager vs
# compiled. Eval: one folded model over the FULL test set, eager vs compiled, at two batch
# sizes. Everything is freed afterwards so the two workers start with the whole GPU.
...
def one_client(Sc, Se, Rc, Re):
    ...
    acc, n = client_update(Sc, Se, Rc, Re, opt, sc, X, Y, 0, hi - lo, CFG, g)
...
def reset():
    unflatten_into(Se, s0, i0, fk, ik); unflatten_into(Re, r0, ri0, fk, ik)
ms, n, sk = one_client(Se, Se, Re, Re)
CAL["train_eager_ms_per_step"] = ms
print(f"train eager   : {ms:.2f} ms per step ({n} steps, {sk} skipped) at batch {CFG['batch']}")
reset()
t0 = time.perf_counter()
Sc, Rc = D._compile_train(Se, Re, CFG, dev, X[:CFG["batch"]].float(), Y[:CFG["batch"]].long())
CAL["compile_seconds"] = time.perf_counter() - t0
CAL["train_backend"] = "compiled" if Sc is not Se else "eager"
if Sc is not Se:
    reset(); one_client(Sc, Se, Rc, Re)                          # warm the graphs
    reset(); ms, n, sk = one_client(Sc, Se, Rc, Re)
...
def build(K, owner, wandb_project, require_resume=False, kernel_sources=(),
    ...
    run_name = f"nilm_{K}c_probe" if probe else f"nilm_{K}c{run_tag}"
    ...
    title = f"NILM-FL VeReMi {K} clients" + (" probe" if probe else "") \
        + (f" {run_tag.strip('_').replace('_', ' ')}" if run_tag else "") \
        + (f" s{session}" if session else "")
    slug = kaggle_slug(title)
    cells = [md(f"""# Lightweight FL (federated mutual learning) on VeReMi NextGen — {K} clients

The federated mutual-learning stage of Li et al., *Lightweight Federated Learning for
On-Device NILM* (Eq. 17–19, Algorithm 1), with **DAGSNet** (395,024 params) as every
model in the system — each client's personalized model w_s, each client's proxy w_r and
the server's aggregate w̄_r — on VeReMi NextGen (16 classes, 66 features). The paper's
MNAS architecture search is deliberately absent; all models share one seeded init.

| from the paper | from `knowledge/` and the owner's decisions |
|---|---|
| per client and batch: L_s = CE(z_s,y) + ℓ_d, L_r = CE(z_r,y) + ℓ_d, ℓ_d = ℓ(y_s,y_r)/(ℓ_s+ℓ_r); upload w_r; server w̄_r = (1/K) Σ w_r^k; every client every round | DAGSNet architecture, 66-feature order, class order, seed 42 |
| ℓ = L2 for the paper's regression output | classifier: ℓ(y_s,y_r) = KL on softmax, one direction per model (w_s ← KL(p_r‖p_s), w_r ← KL(p_s‖p_r)), other side detached; weight 1/(ℓ_s+ℓ_r) stop-gradient |
| | batch {sc['batch']}, {rounds} rounds × 1 local epoch, AdamW / wd {PAPER['weight_decay']} re-created per client per round, lr {PAPER['lr']} → {PAPER['lr_min']} cosine per round, clip 1.0, fp16 AMP |

**Evaluation:** every round, every client's personalized model w_s^k(x) on the full
10,761,343-row test set — 10 metrics per client, mean/std/min/max over all {K} clients —
plus the aggregated proxy w̄_r as one extra model (`global_*` columns, never mixed into
the client mean).

**Deviations, all deliberate:** no MNAS, one architecture for every model; KL instead of
L2 and a stop-gradient adaptive weight (the paper is a regression method); unweighted 1/K
aggregation as in Algorithm 1 (not FedAvg's n_k weights); AdamW with a per-round cosine
schedule (the paper names neither); the final local fine-tuning step of the paper's
framework is skipped (w_s trains on local labels every round). Per-round output is
**weights only** (w̄_r + every w_s^k as state_dict tensors); `proj/ckpt.py::load_weights`
rebuilds them at any round, and the last cell re-derives every published metric from the
...
sys.path.insert(0, "/kaggle/working")"""),
    ...
    # --- federated mutual learning: owner's decisions 2026-09-15 (paper: no optimizer
    #     or rate given); LR cosine per round lr -> lr_min (proj/nilm.py::lr_at)
...
if rows:
    ...
    for k in METRIC_KEYS: print(f"  {k:<20} {float(fin[k]):.6f}   proxy w_bar_r {float(fin['global_' + k]):.6f}")
...
if len(sec) < 2:
    ...
    print(f"timing   : rounds {[round(x) for x in sec[-3:]]}s | steady {steady:.0f}s/round "
          f"= train {tr:.0f}s + eval {ev:.0f}s ({hist[-1]['evaluated']} clients + proxy) + commit")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--wandb-project", default="nilm-veremi")
    ...
    for K in a.clients:
        nb, meta = build(K, a.owner, a.wandb_project, a.require_resume, a.kernel_source,
                         ...
                         a.session)
        name = meta["code_file"]
        out = ROOT / "papers/nilm-li-2024/notebook" / (

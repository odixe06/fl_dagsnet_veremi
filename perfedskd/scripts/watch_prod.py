#!/usr/bin/env python
"""Poll W&B (and optionally the Kaggle kernel status) for the production NILM-FL runs.

    python scripts/watch_prod.py perfedskd_20c perfedskd_50c perfedskd_100c
    python scripts/watch_prod.py perfedskd_20c --kernel minhtriethihi/perfedskd-veremi-20-clients \
        --every 300
...
Stop signals to watch for (CONTEXT.md): backend `eager`, skips growing round over round,
...
COLS = ("f1_macro", "f1_macro_min", "f1_macro_max", "accuracy", "global_f1_macro",
        "global_accuracy", "lr", "ce_s_client_mean", "ce_r_client_mean", "kl_s_client_mean",
        "kl_r_client_mean", "skipped", "steps", "train_sec", "eval_sec", "seconds",
        "vram_train_gb", "evaluated")
SKILL = Path(__file__).resolve().parents[2] / ".claude/skills/kaggle-training-notebook/scripts"
...
def main():
    ...
    ap.add_argument("--project", default="21522798-uit/perfedskd-veremi")

#!/usr/bin/env python
"""Static gate on the generated PerFed-SKD notebooks. Cheap checks that each cost a Kaggle version."""
...
NB = ROOT / "papers/perfedskd-singh-2025/notebook"
...
for K, probe, dirname in targets:
    ...
    chk("build_model=build_model" in flow and "expect_params=N_PARAMS" in flow,
        "the verifier must rebuild the server aggregate and every personalized model")
    chk("verify_run(" in flow, "summary must re-derive the metrics from the artifacts")
    # The selection rule lives in proj/perfedskd.py; CFG only declares it so the
    # fingerprint can see it. The driver asserts they agree, and this is the static half.
    chk("SELECT_METRIC" in embedded.get("driver", ""),
        "the driver must compare cfg['select_metric'] against the module constant")
    ...
    chk(re.fullmatch(rf"perfedskd_{K}c(_probe|_[a-z0-9]+)?", name) is not None,
        f"run_name {name!r} is not perfedskd_{K}c with an optional tag")

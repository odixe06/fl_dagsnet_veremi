#!/usr/bin/env python
"""Numbers and figures for the PerFed-SKD report, rebuilt from pulled run artifacts only.

    python scripts/report_data.py --out papers/perfedskd-singh-2025/report_data \
        20c=papers/perfedskd-singh-2025/runs/merged/perfedskd_20c \
        50c=papers/perfedskd-singh-2025/runs/merged/perfedskd_50c \
        100c=papers/perfedskd-singh-2025/runs/merged/perfedskd_100c
...
Per scenario (K = 20/50/100 clients) it writes, under --out:
  history_Kc.csv          every round, every column of the run's history.csv (mean/std/min/max
                          of the 10 metrics over the N personalized models ω_m, the 10
                          global_* metrics of the server server aggregate ω^t, loss terms, lr, timing, skips)
  rounds_Kc.md            the same as a markdown table: all 10 metrics (mean over clients) for
                          EVERY round plus the aggregate f1_macro/accuracy and |S|, tau — the reporting contract
  ...
  per_class_final_Kc.csv  per class at the last round: support, P/R/F1 as the mean over clients,
                          on the pooled (summed) client confusion, and of the server aggregate ω^t
  confusion_final_Kc.csv  pooled confusion matrix (sum over clients) of the last round
  confusion_global_Kc.csv confusion matrix of the server aggregate ω^t at the last round
  summary.json            headline numbers of all scenarios: final round, post-hoc peak, aggregate, selection,
                          timing, sessions, config read from reports/manifest.json
and the figures convergence.png, skd_loss.png, client_spread.png, per_class_f1_Kc.png,
...
def write_history(d, out, tag, rows):
    ...
    lines = ["| round | lr | " + " | ".join(METRICS) + " | f1_macro std | min | max | "
             "global f1_macro | global acc | |S| | tau | skip | train s | eval s |",
             "|---:|---:|" + "---:|" * len(METRICS) + "---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for r in hist:
        lines.append(f"| {r['round']} | {float(r['lr']):.2e} | "
                     ...
                     + f" | {float(r['f1_macro_std']):.4f} | {float(r['f1_macro_min']):.4f} | "
                       ...
                       f"{float(r['global_accuracy']):.4f} | {int(r['n_selected'])} | "
                       f"{float(r['tau']):.4f} | {int(r['skipped'])} | "
                       f"{float(r['train_sec']):.0f} | {float(r['eval_sec']):.0f} |")
    (out / f"rounds_{tag}.md").write_text(
        f"# {tag}: 10 metrics per round (mean over all M personalized models omega_m, full "
        "10 761 343-row test set); global = the server aggregate omega^t on the same test; "
        "|S| = devices selected for THIS round, tau = the threshold computed from it\n\n"
...
def write_readme(out, summary):
    ...
    tags = list(summary)
    L = ["# Số liệu cho REPORT.md — PerFed-SKD trên VeReMi / DAGSNet", "",
         ...
         "ω_m, mỗi model đo trên **toàn bộ 10 761 343 dòng test toàn cục**; metric **global** là của "
         "một model ω^t (server) trên cùng tập test. Caveat bắt buộc: `docs/rebuild.md` §2 (D1–D13) và §8.", ""]
    ...
    L += ["", "## 2. Round cuối — đủ 10 metric (mean trên client; std/min/max của f1_macro; server aggregate ω^t)", "",
          "accuracy = precision_micro = recall_micro = recall_weighted = f1_micro (đồng nhất thức đa lớp đơn nhãn).", "",
          "| metric | " + " | ".join(f"{t} ω_m (r{summary[t]['final_round']}) | {t} ω^t" for t in tags) + " |",
          "|---|" + "---:|---:|" * len(tags)]
    for m in METRICS:
    ...
    L += ["", "## 3. Hình dạng đường cong (round 1 → đỉnh hậu kiểm → cuối)", "",
          ...
          "| | " + " | ".join(tags) + " |", "|---|" + "---:|" * len(tags)]
    L.append("| f1_macro ω_m round 1 | " + " | ".join(f"{summary[t]['round1']['f1_macro']:.4f}" for t in tags) + " |")
    L.append("| f1_macro ω_m đỉnh (round) | " + " | ".join(f"{summary[t]['peak_f1_macro_posthoc']['f1_macro']:.4f} (r{summary[t]['peak_f1_macro_posthoc']['round']})" for t in tags) + " |")
    L.append("| f1_macro ω_m cuối | " + " | ".join(f"{summary[t]['final']['f1_macro']['mean']:.4f}" for t in tags) + " |")
    L.append("| chênh so đỉnh | " + " | ".join(f"−{summary[t]['drop_from_peak_pct']:.1f} %" for t in tags) + " |")
    L.append("| accuracy ω_m round 1 → cuối | " + " | ".join(f"{summary[t]['round1']['accuracy']:.4f} → {summary[t]['final']['accuracy']['mean']:.4f}" for t in tags) + " |")
    L.append("| f1_macro ω^t round 1 → 2 → cuối | " + " | ".join(f"{summary[t]['global_round1']['f1_macro']:.4f} → {summary[t]['global_round2']['f1_macro']:.4f} → {summary[t]['final_global']['f1_macro']:.4f}" for t in tags) + " |")
    L.append("| accuracy ω^t round 1 → cuối | " + " | ".join(f"{summary[t]['global_round1']['accuracy']:.4f} → {summary[t]['final_global']['accuracy']:.4f}" for t in tags) + " |")
    ...
    L += ["", "## 5. Chọn thiết bị và truyền tin (quy tắc τ của PerFed-SKD)", "",
          "|S_t| = số device được chọn ở round t (round 1 = M theo định nghĩa); comm_saving = 1 − |S_t|/M "
          "của chi phí giao thức (|S_t| lên + |S_t| xuống, **tính**, không đo — D12).", "",
          "| | " + " | ".join(tags) + " |", "|---|" + "---:|" * len(tags)]
    L.append("| \\|S\\| mean (min–max, bỏ round 1) | " + " | ".join(
        f"{np.mean(summary[t]['selection']['n_selected'][1:]):.1f} ({min(summary[t]['selection']['n_selected'][1:])}–{max(summary[t]['selection']['n_selected'][1:])}) / {summary[t]['n_clients']}" for t in tags) + " |")
    L.append("| \\|S\\|/M mean (bỏ round 1) | " + " | ".join(
        f"{np.mean(summary[t]['selection']['n_selected'][1:]) / summary[t]['n_clients']:.3f}" for t in tags) + " |")
    L.append("| comm_saving mean (mọi round) | " + " | ".join(f"{summary[t]['selection']['comm_saving_mean']:.3f}" for t in tags) + " |")
    L.append("| truyền lên / xuống tổng (MiB) | " + " | ".join(
        f"{summary[t]['selection']['comm_up_mib_total']:.0f} / {summary[t]['selection']['comm_down_mib_total']:.0f}" for t in tags) + " |")
    L.append("| τ round 1 → cuối | " + " | ".join(
        f"{summary[t]['selection']['tau'][0]:.4f} → {summary[t]['selection']['tau'][-1]:.4f}" for t in tags) + " |")
    L += ["", "## 6. Kiểm soát chất lượng pipeline", "", "| | " + " | ".join(tags) + " |", "|---|" + "---:|" * len(tags)]
...
def main():
    ...
    ap.add_argument("runs", nargs="+", help="tag=run_dir, e.g. 20c=.../merged/perfedskd_20c")
    ...
    for spec in a.runs:
        ...
        # per class, last round: mean over clients + pooled confusion + aggregate
        ...
        with open(a.out / f"per_class_final_{tag}.csv", "w", newline="") as f:
            ...
            w.writerow(["idx", "class", "support", "share", "precision_mean", "recall_mean",
                        ...
                        "recall_pooled", "f1_pooled", "precision_global", "recall_global", "f1_global"])
        ...
        summary[tag] = {
            ...
            "loss": {"round1": loss_at(rows[0]), "round2": loss_at(rows[1]) if len(rows) > 1 else None,
                     "final": loss_at(last)},
            # PerFed-SKD's own result: how many devices the threshold rule selected, round by
            # round, and what that would have saved on the wire against broadcasting to all M.
            "selection": {
                "n_selected": [int(r["n_selected"]) for r in hist],
                "tau": [float(r["tau"]) for r in hist],
                "n_selected_mean": float(np.mean([int(r["n_selected"]) for r in hist])),
                "n_selected_min": int(min(int(r["n_selected"]) for r in hist)),
                "n_selected_max": int(max(int(r["n_selected"]) for r in hist)),
                "comm_saving_mean": float(np.mean([float(r["comm_saving"]) for r in hist])),
                "comm_up_mib_total": float(sum(float(r["comm_up_mib"]) for r in hist)),
                "comm_down_mib_total": float(sum(float(r["comm_down_mib"]) for r in hist)),
                "select_metric": hist[0].get("select_metric", "accuracy")},
        ...
        # figure: per-class f1 (mean over clients), ordered by support; pooled + aggregate as markers
        order = np.argsort(-support)
        fig, ax = plt.subplots(figsize=(7.5, 3.6))
        arr = np.array([np.mean([x[2] for x in pc[i]]) for i in range(len(classes))])
        ax.bar(range(len(classes)), arr[order], color=SERIES[tag], width=0.7, label=f"mean over {N} ω_m")
        ax.plot(range(len(classes)), pf[order], "o", color="#1a1a19", ms=4, label="pooled confusion")
        ax.plot(range(len(classes)), gf[order], "D", color="#8a3ffc", ms=4, label="server aggregate ω^t")
        ax.set_xticks(range(len(classes)))
        ax.set_xticklabels([f"{classes[i]}\n{support[i]/support.sum()*100:.1f}%" for i in order],
                           rotation=60, ha="right", fontsize=7)
        ax.set_ylim(0, 1.15); ax.set_yticks(np.linspace(0, 1, 6)); ax.set_ylabel("F1 (round %d)" % last["round"])
    ...
    # convergence: f1_macro and accuracy (personalized mean ± std, aggregate dashed), LR
    fig, axes = plt.subplots(1, 3, figsize=(12.5, 3.6))
    ends = {0: [], 1: []}
    for tag, hist in hists.items():
        r = np.array([int(x["round"]) for x in hist])
        for i, (ax, m) in enumerate(zip(axes[:2], ("f1_macro", "accuracy"))):
            ...
            ax.plot(r, mu, color=SERIES[tag], lw=2, label=f"{tag} ω_m ({len(r)} round)")
            ax.plot(r, g, color=SERIES[tag], lw=1.3, ls="--", label=f"{tag} ω^t")
            ends[i] += [[mu[-1], mu[-1], r[-1], f"{tag} ω_m", "#1a1a19"], [g[-1], g[-1], r[-1], f"{tag} ω^t", "#5a5a58"]]
    for i, ax in enumerate(axes[:2]):     # end labels, spread so the six never overlap
        e = sorted(ends[i])
        for j in range(1, len(e)):
            e[j][1] = max(e[j][1], e[j - 1][1] + 0.045)
        for v, yl, x, name, c in e:
            ax.text(x + 1, yl, f"{name} {v:.3f}", color=c, fontsize=6.5, va="center", clip_on=False)
    longest = max(hists.values(), key=len)
    axes[2].plot([int(x["round"]) for x in longest], [float(x["lr"]) for x in longest],
                 color="#1a1a19", lw=2)
    axes[0].set_title("f1_macro — ω_m mean ± std (solid), ω^t (dashed)", fontsize=8.5)
    axes[1].set_title("accuracy — ω_m mean ± std (solid), ω^t (dashed)", fontsize=8.5)
    ...
    axes[0].legend(frameon=False, fontsize=6.5, ncol=2, loc="lower right")
    fig.tight_layout(w_pad=7); fig.savefig(a.out / "convergence.png", bbox_inches="tight"); plt.close(fig)

    # mutual-learning loss terms per round
    fig, axes = plt.subplots(1, 2, figsize=(9, 3.2))
    for tag, hist in hists.items():
        r = [int(x["round"]) for x in hist]
        axes[0].plot(r, [float(x["ce_client_mean"]) for x in hist], color=SERIES[tag], lw=2, label=f"{tag} ce")
        axes[1].plot(r, [float(x["kd_client_mean"]) for x in hist], color=SERIES[tag], lw=2, label=f"{tag} kd")
    axes[0].set_title("cross-entropy CE(z, y), client mean")
    axes[1].set_title("self-distillation KL(p_teacher ‖ p_student), λ = 1, client mean")
    for ax in axes:
        ax.set_xlabel("round"); ax.set_xlim(0, 51); ax.legend(frameon=False, fontsize=6.5, ncol=3)
    fig.tight_layout(); fig.savefig(a.out / "skd_loss.png"); plt.close(fig)
    ...
    bp = ax.boxplot(data, widths=0.5, patch_artist=True, showfliers=False,
                    medianprops={"color": "#1a1a19", "lw": 1.2})
    ...
    ax.set_ylabel("f1_macro per client"); ax.set_title("Per-client f1_macro, last round (purple bar = aggregate ω^t)", fontsize=8.5)
    fig.tight_layout(); fig.savefig(a.out / "client_spread.png"); plt.close(fig)

    # device selection: |S_t|/M and the threshold tau_t
    fig, axes = plt.subplots(1, 2, figsize=(9, 3.2))
    for tag, hist in hists.items():
        r = [int(x["round"]) for x in hist]
        M = summary[tag]["n_clients"]
        axes[0].plot(r, [int(x["n_selected"]) / M for x in hist], color=SERIES[tag], lw=1.6, label=tag)
        axes[1].plot(r, [float(x["tau"]) for x in hist], color=SERIES[tag], lw=2, label=tag)
    axes[0].axhline(0.5, color="#8a8a86", lw=0.8, ls=":")
    axes[0].set_title("|S_t| / M — share of devices receiving ω^{t-1} and uploading", fontsize=8.5)
    axes[1].set_title("τ_t = mean accuracy of all M devices (Alg. 1 line 11)", fontsize=8.5)
    axes[0].set_ylim(0, 1.05)
    for ax in axes:
        ax.set_xlabel("round"); ax.set_xlim(0, 51)
    axes[0].legend(frameon=False)
    fig.tight_layout(); fig.savefig(a.out / "selection.png"); plt.close(fig)
    ...
    for t, s in summary.items():
        print(f"{t}: rounds {s['rounds_completed']}/{s['rounds_planned']} "
              ...
              f"global {s['final_global']['f1_macro']:.4f} acc {s['final']['accuracy']['mean']:.4f} "

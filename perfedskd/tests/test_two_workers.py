...
the server aggregate on the last worker) must produce the SAME weights and metrics as 1
worker, bit for bit. Small fixture so two worker processes plus the parent stay under the 8 GB watchdog."""
...
def main():
    ...
    for k in ("f1_macro", "accuracy", "f1_macro_std", "global_f1_macro", "global_accuracy",
              "tau", "loss_client_mean", "ce_client_mean", "kd_client_mean"):
        assert abs(h1[k] - h2[k]) < 1e-12, (k, h1[k], h2[k])
    # the device subset is a result too: a schedule-dependent tau would reshuffle it
    assert w1["selected"] == w2["selected"] and w1["selected_next"] == w2["selected_next"]
    shutil.rmtree(tmp)
    print(f"\n1 worker vs 2 workers: max|dweight| = {dmax}, f1_macro {h1['f1_macro']:.6f} == "
          f"{h2['f1_macro']:.6f}, aggregate {h1['global_f1_macro']:.6f} == "
          f"{h2['global_f1_macro']:.6f}, S identical ({len(w1['selected_next'])} devices)"

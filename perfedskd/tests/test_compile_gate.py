...
def gate(wrapper, dropout=0.1):
    ...
    Se, Ve = build_model(cfg), build_model(cfg)
    xb = torch.randn(B, 66) * 2; yb = torch.randint(0, 16, (B,))
    real = torch.compile
    torch.compile = lambda m, **kw: wrapper(m)
    try:
        Sc, Vc = driver._compile_train(Se, Ve, cfg, torch.device("cpu"), xb, yb)
    finally:
        torch.compile = real
    ps = [m.p for m in list(Se.modules()) + list(Ve.modules()) if isinstance(m, torch.nn.Dropout)]
    # the student must come back trainable and in train(); the teacher must come back in
    # eval() -- a gate that leaves V_m in train() would answer every batch differently
    trainable = (Se.training and not Ve.training
                 and all(p.requires_grad for p in Se.parameters()))
    return (Sc is not Se) and (Vc is not Ve), ps, trainable

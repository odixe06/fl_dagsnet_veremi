"""End-to-end on REAL VeReMi rows: prepack -> 3 PerFed-SKD rounds -> per-client and
server-aggregate metrics -> device selection chain -> crash recovery -> next-session
...
sys.path.insert(0, str(ROOT / "papers/perfedskd-singh-2025"))
...
from proj.perfedskd import lr_at, ACC_KEYS
...
def main():
    ...
    for r in range(1, R + 1):
        ...
        for k in METRIC_KEYS:
            ...
            assert f"global_{k}" in mj and k in mj["global"], f"round {r}: no global {k}"
        ...
        # Algorithm 1: round 1 selects everybody; afterwards S_t is exactly what round
        # t-1 committed as S_{t+1}, and the per-client log agrees with it.
        sel = mj["selected"]
        assert mj["n_selected"] == len(sel) and lg["selected"] == sel
        if r == 1:
            assert sel == list(range(N)), f"round 1 selected {sel}"
        else:
            prev = json.loads((d / "metrics" / f"round_{r-1:03d}.json").read_text())
            assert sel == prev["selected_next"], (sel, prev["selected_next"])
        assert all(bool(c["selected"]) == (c["cid"] in set(sel)) for c in lg["clients"])
        accs = [c["accuracy"] for c in mj["clients"]]
        assert abs(mj["tau"] - sum(accs) / len(accs)) < 1e-12, "tau != mean client accuracy"
        assert mj["selected_next"] == [c for c in range(N) if accs[c] < mj["tau"]]
        assert abs(mj["comm_saving"] - (1 - len(sel) / N)) < 1e-12
    ...
    print(f"   round 1 f1_macro mean={hist[0]['f1_macro']:.6f} global={hist[0]['global_f1_macro']:.6f} | "
          f"round {R} mean={hist[-1]['f1_macro']:.6f} std={hist[-1]['f1_macro_std']:.4f} "
          f"global={hist[-1]['global_f1_macro']:.6f} | ce={hist[-1]['ce_client_mean']:.4f} "
          f"kd={hist[-1]['kd_client_mean']:.4f} | |S| per round "
          f"{[h['n_selected'] for h in hist]} tau {[round(h['tau'], 4) for h in hist]} "
          f"| lr per round {[round(h['lr'], 7) for h in hist]}")
    # Round 1's teacher is omega^0 for every device, so the KD term is 0 at the FIRST STEP
    # (tests/test_units.py proves that) -- but NOT over the round: a student leaving a
    # random-init teacher diverges from it fast, so round 1's mean KD is the LARGEST of the
    # run, not the smallest. What must hold every round is that the term is finite and
    # positive, and that S_1 is everybody.
    assert all(0.0 < h["kd_client_mean"] < 1e3 for h in hist), [h["kd_client_mean"] for h in hist]
    assert hist[0]["n_selected"] == N and hist[0]["comm_saving"] == 0.0
    h = hist[-1]
    ...
    assert w1["selected"] == list(range(N)) and w2["selected"] == w1["selected_next"]
    ...
    print(f"   every personalized model and the aggregate moved each round; clients differ; "
          f"weights file has the {len(set(w2))} keys incl. selected/selected_next/tau")

    # weights file is weights-only and rebuilds the proxy and every w_s with strict=True
    Gm, Ms, meta = C.load_weights(d / f"weights/round_{R:03d}.pt", build_model, N_PARAMS)
    assert len(Ms) == N and meta["round"] == R
    assert meta["selected_next"] == json.loads(
        (d / "metrics" / f"round_{R:03d}.json").read_text())["selected_next"]
    print(f"   round-{R} weights rebuild: aggregate + {N} x {N_PARAMS:,}, weights_only=True, "
    ...
    assert ok, "\n".join(lines)
    print("   verify_run: per-client + aggregate metrics == confusion == json == csv; "
          "tau and S recomputed from the matrices; preds rebuild the CMs")

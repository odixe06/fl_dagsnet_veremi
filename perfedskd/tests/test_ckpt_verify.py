...
CFGD = {**CFG, "n_features": 66, "lr": 1e-3, "lr_schedule": "cosine", "lr_min": 1e-5,
        ...
        "lam": 1.0, "select_metric": "accuracy", "preds_rounds": [2]}
...
def write_run(d, cfg, seed=0, rounds=2, mark_upto=None):
    ...
    G, Ms = build_model(cfg), {c: build_model(cfg) for c in range(N)}
    sel = list(range(N))          # Algorithm 1: round 1 selects every device
    for r in range(1, rounds + 1):
        with torch.no_grad():
            for p in G.parameters(): p.add_(torch.randn_like(p) * 0.01)
            for c in range(N):
                for p in Ms[c].parameters(): p.add_(torch.randn_like(p) * 0.01)
        ...
        sel_next = select_clients(acc_by_cid, tau)
        row.update({"n_selected": len(sel), "tau": tau, "n_selected_next": len(sel_next),
                    "select_metric": SELECT_METRIC,
                    "comm_down_mib": 1.0 * len(sel), "comm_up_mib": 1.0 * len(sel),
                    "comm_saving": 1.0 - len(sel) / N})
        row.update({f"{k}_client_mean": 0.5 for k in ACC_KEYS}); row["seconds"] = 1.0
        C.save_round_weights(C.cpu_sd(G), {c: C.cpu_sd(Ms[c]) for c in range(N)}, r, cfg,
                             {k: row[k] for k in METRIC_KEYS}, gm, d, sel, sel_next, tau)
        ...
        for c in range(N):
            ...
            logs.append({"cid": c, "n_k": 100, "lr": lr_at(cfg, r), "steps": s, "applied": s,
                         "skipped": 0, "nonfinite": 0, "selected": c in set(sel),
                         **{k: 0.5 for k in ACC_KEYS}})
        (d / "logs" / f"round_{r:03d}.json").write_text(json.dumps(
            {"clients": logs, "selected": sel, "selected_next": sel_next, "tau": tau}))
        (d / "metrics" / f"round_{r:03d}.json").write_text(json.dumps(
            {**row, "selected": sel, "selected_next": sel_next, "clients": per_client,
...
def main():
    ...
    assert set(raw) == {"round", "global", "clients", "cfg", "fingerprint", "prev_sha", "metrics",
                        "global_metrics", "selected", "selected_next", "tau"}
    # selected_next must survive weights_only=True: plain ints, never a numpy array
    assert all(type(c) is int for c in raw["selected_next"]) and type(raw["tau"]) is float
    ...
    assert rs["round"] == 2 and "rng" in rs and rs["selected_next"] == raw["selected_next"]
    Gm, Ms_, ck = C.load_weights(d / "weights/round_002.pt", build_model, N_PARAMS)
    assert len(Ms_) == N and not Gm.training and not any(m.training for m in Ms_.values())
    assert ck["selected_next"] == raw["selected_next"]
    G1, S1, _ = C.load_weights(d / "weights/round_002.pt", build_model, clients=[1])
    x = torch.randn(4, 66)
    assert torch.equal(Gm(x), G1(x)) and list(S1) == [1]
    # a checkpoint with no usable selected_next cannot be resumed from: the next round's
    # device subset would have to be invented
    bad = torch.load(d / "weights/round_002.pt", map_location="cpu", weights_only=True)
    bad["selected_next"] = None
    C.atomic_save(bad, tmp / "nosel.pt")
    try:
        C.load_weights(tmp / "nosel.pt", build_model, N_PARAMS); raise AssertionError
    except RuntimeError as e:
        assert "selected_next" in str(e)
    print(f"2. weights: tensors only, BN buffers included, weights_only=True, selected_next/tau "
          f"are plain types, strict rebuild of aggregate + {N} personalized models, "
    ...
    for k, v in (("lr", 0.01), ("local_epochs", 2), ("clip", 0.5),
                 ...
                 # lam changes the objective, select_metric changes the algorithm
                 ("lam", 0.5), ("select_metric", "f1_macro")):
    ...
    print("3. fingerprint: 15 scientific keys move it, 5 operational keys do not, missing key raises"); n += 1
    ...
    def m_tau(e):
        j = _rj(e); j["tau"] = 0.999; _wj(e, j)
    def m_tau_w(e):
        w = torch.load(e / "weights/round_002.pt", map_location="cpu", weights_only=True)
        w["tau"] = 0.999; C.atomic_save(w, e / "weights/round_002.pt")
    def m_selnext(e):
        j = _rj(e); j["selected_next"] = list(range(N)); j["n_selected_next"] = N; _wj(e, j)
    def m_chain(e):
        # round 2 claims a subset round 1 never committed: the selection history is broken.
        # The replacement is derived from round 1's actual S_2 so the case cannot become a
        # no-op when the random fixture happens to produce the subset we hard-coded.
        prev = json.loads((e / "metrics/round_001.json").read_text())["selected_next"]
        other = next(x for x in ([c] for c in range(N + 1)) if x != prev)
        other = [c for c in other if c < N] or list(range(N))
        assert other != prev, (other, prev)
        j = _rj(e); j["selected"] = other; j["n_selected"] = len(other)
        w = torch.load(e / "weights/round_002.pt", map_location="cpu", weights_only=True)
        w["selected"] = other; C.atomic_save(w, e / "weights/round_002.pt")
        lg = json.loads((e / "logs/round_002.json").read_text()); lg["selected"] = other
        for c in lg["clients"]: c["selected"] = (c["cid"] in set(other))
        (e / "logs/round_002.json").write_text(json.dumps(lg)); _wj(e, j)
    def m_selflag(e):
        lg = json.loads((e / "logs/round_002.json").read_text())
        lg["clients"][0]["selected"] = not lg["clients"][0]["selected"]
        (e / "logs/round_002.json").write_text(json.dumps(lg))
    ...
    for label, fn in (("NaN per-client metric", m_nan), ("mean overwritten", m_mean),
                      ("global metric overwritten in json", m_global),
                      ("global metric overwritten in the row", m_global_row),
                      ("global metric overwritten in the weights file", m_global_w),
                      ("global confusion edited", m_gcm),
                      ("history.csv row dropped", m_csv), ("clients.csv value edited", m_ccsv),
                      ("per-class f1 = 999", m_pc), ("global per-class recall = 999", m_gpc),
                      ("client missing from the log", m_log_missing_client),
                      ("client applied 0 steps", m_steps),
                      ("client applied a non-finite step", m_nonfin),
                      ("predictions edited", m_preds), ("global predictions edited", m_gpreds),
                      ...
                      ("tau overwritten in json", m_tau),
                      ("tau overwritten in the weights file", m_tau_w),
                      ("selected_next widened to every device", m_selnext),
                      ("selection chain broken (S_t != previous S_{t+1})", m_chain),
                      ("a client log's selected flag flipped", m_selflag),
                      ("global confusion file missing", m_no_gcm)):
        case(label, fn)
    ...
    assert (dest / "confusion/global_001.npy").is_file(), "global confusion not imported"
    assert os.access(dest / "weights/round_001.pt", os.W_OK)
    rows = (dest / "history.csv").read_text().splitlines()
    assert len(rows) == 2, rows
    print("5. import from a read-only mount: only the source's committed round 1 (incl. global CM), "
    ...
    merged = tmp / "merged"
    r = subprocess.run([sys.executable, str(Path(__file__).resolve().parents[2] /
                        ".claude/skills/kaggle-training-notebook/scripts/merge_sessions.py"),

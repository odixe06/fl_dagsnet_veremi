...
import os, wandb
try:
    r = wandb.Api(timeout=60).run(f"21522798-uit/lwfednids-veremi/{os.environ['RN']}")
    h = r.history(samples=100, pandas=True)
    keep = [c for c in ("f1_macro","accuracy","loss_client_mean","train_sec","eval_sec","seconds","skipped","vram_train_gb") if c in h.columns]

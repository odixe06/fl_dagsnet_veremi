#!/usr/bin/env python
"""Generate one Lightweight-Fed-NIDS training notebook per client scenario.

The notebook body is assembled from papers/lwfednids-bouayad-2024/proj/*.py, which are the
...
supported path; never hand-edit the .ipynb.
    ...
    python scripts/gen_notebook.py --owner minhtran0601                     # 20c 50c 100c
    python scripts/gen_notebook.py --owner minhtran0601 --clients 20 --probe --max-hours 2
    python scripts/gen_notebook.py --owner X --clients 100 --session 2 --require-resume \\
        --kernel-source X/lightweight-fed-nids-veremi-100-clients --max-hours 11.75
"""
import argparse, json, re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
PROJ = ROOT / "papers/lwfednids-bouayad-2024/proj"
RUNTIME = json.loads((ROOT / "knowledge/runtime.json").read_text())
META = json.loads((ROOT / "knowledge/meta.json").read_text())
MODULES = ("model", "prune", "ckpt", "metrics", "data", "lwfednids", "evaluate", "driver",
           "verify")
TORCH_PRUNING = "torch-pruning==1.6.1"      # server-side mask computation only; --no-deps

# batch from knowledge/dataset.md §4 and the owner's instruction of 2026-09-22. Every
# client trains in every round (Algorithm 6 lines 6-8 loop over all N).
...
# Owner's decisions of 2026-09-22 (docs/rebuild.md): sparsity 0.7 only; AdamW / wd 1e-4 as
# in knowledge/architecture.md; 50 rounds x 1 local epoch; per-round cosine LR
# 1e-3 -> 1e-5 (proj/lwfednids.py::lr_at); unweighted 1/N aggregate (Eq. 9).
PAPER = dict(sparsity=0.7, prune_importance="group_l1",
             lr=1e-3, lr_schedule="cosine", lr_min=1e-5, weight_decay=1e-4,
             rounds=50, local_epochs=1)
WANDB_ENTITY = "21522798-uit"
...
def build(K, owner, wandb_project, require_resume=False, kernel_sources=(),
    ...
    run_name = f"lwfednids_{K}c_probe" if probe else f"lwfednids_{K}c{run_tag}"
    ...
    title = f"Lightweight-Fed-NIDS VeReMi {K} clients" + (" probe" if probe else "") \
        + (f" {run_tag.strip('_').replace('_', ' ')}" if run_tag else "") \
        + (f" s{session}" if session else "")
    slug = kaggle_slug(title)
    cells = [md(f"""# Lightweight-Fed-NIDS on VeReMi NextGen — {K} clients, sparsity {PAPER['sparsity']}
...
unpruned → 35,891 at sparsity {PAPER['sparsity']}, as computed and printed below); the paper's
...
|---|---|
| §III-B-1-b: mask from the initialized weights, no data; Torch-Pruning / DepGraph; L1 group norm; "sparsity" = per-layer channel ratio | sparsity **{PAPER['sparsity']}** only; uniform per-layer ratio; final Linear (16 logits) not pruned; pruned channels physically removed; the plan (kept indices) is in `CFG` and in every weights file |
| Alg. 5: θ_j ← θ; E local epochs of gradient descent, no rate given | 1 local epoch, batch {sc['batch']}; AdamW, wd {PAPER['weight_decay']}, re-created per client per round; lr {PAPER['lr']} → {PAPER['lr_min']} cosine per round; clip 1.0; fp16 AMP; seed 42 |
...
| T = 5 rounds in the paper | {rounds} rounds |
...
plus the mean / std / min / max over the {K} clients of each client's training loss and
...
confusion matrices on disk."""),
    ...
    code(f"""import os, subprocess, sys, time, torch
...
r = subprocess.run([sys.executable, "-m", "pip", "install", "--quiet", "--no-deps",
                    "{TORCH_PRUNING}"], capture_output=True, text=True)
...
sys.path.insert(0, "/kaggle/working")"""),
    ...
    sparsity={PAPER["sparsity"]}, prune_importance="{PAPER["prune_importance"]}",
    ...
    preds_rounds={[] if probe else [rounds]},   # (10.76 M,) uint8 per listed round
...
for k, v in CFG.items(): print(f"{{k:>20}} = {{v}}")''')]
    ...
    cells += [code('''# ---- server initialization (§III-B-1): theta_0 from the seed, the mask from theta_0
...
if __name__ == "__main__":
    ...
    ap.add_argument("--wandb-project", default="lwfednids-veremi")
    ...
    for K in a.clients:
        nb, meta = build(K, a.owner, a.wandb_project, a.require_resume, a.kernel_source,
                         ...
                         a.session)
        name = meta["code_file"]
        out = ROOT / "papers/lwfednids-bouayad-2024/notebook" / (

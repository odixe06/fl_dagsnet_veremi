#!/usr/bin/env python
"""Static gate on the generated Lightweight-Fed-NIDS notebooks. Cheap checks that each cost a Kaggle version."""
...
CFG_LATE_KEYS = {"data_id", "n_test", "content_id", "backend", "backend_eval", "startup_seconds",
                 # filled by the server-initialization cell (compute_plan), before the fingerprint
                 "prune_plan", "plan_id", "n_params"}
...
NB = ROOT / "papers/lwfednids-bouayad-2024/notebook"
...
for K, probe, dirname in targets:
    ...
    # A pushed notebook is a record of what ran and is never regenerated; proj/*.py moves on
    # (a later session's resume route, say) and the record is expected to lag it. The
    # marker is written by the push step; everything else is still checked.
    frozen = (d / "PUSHED").is_file()
    for m, body in embedded.items():
        want = (PROJ / f"{m}.py").read_text().rstrip()
        if frozen and body != want:
            print(f"  note {tag}: proj/{m}.py has moved on since the push "
                  f"({(d / 'PUSHED').read_text().strip()}); not regenerated by design")
            continue
    ...
    # -- torch must never be reinstalled on a Kaggle GPU image. Token-based: the notebook
    #    installs `torch-pruning` (allowed, --no-deps) and a substring test would flag it.
    #    A list-form call spans lines, so bracketed text is joined before the line scan.
    flat = re.sub(r"\[[^\]]*\]", lambda m: m.group(0).replace("\n", " "), joined)
    for mm in re.finditer(r"pip[^\n]*?install([^\n\]]*)", flat):
        toks = re.findall(r"[\w\-\.=<>!~]+", mm.group(1).replace('"', " ").replace("'", " "))
        chk(not any(re.fullmatch(r"torch(vision|audio)?([=<>!~].*)?", t) for t in toks),
            f"reinstalling torch breaks CUDA on this image: {mm.group(0)[:60]!r}")
    chk("--no-deps" in src[0] and "torch-pruning" in src[0],
        "torch-pruning must be installed with --no-deps in the first cell")
    ...
    missing = [k for k in FINGERPRINT_KEYS if k not in cfg and k not in ("data_id", "plan_id")]
    ...
    # -- the mask is computed by the server BEFORE the fingerprint resolves a checkpoint,
    #    from the seed alone (no data), and its id enters the fingerprint
    chk("compute_plan(CFG" in flow and 'CFG["plan_id"]' in flow and 'CFG["n_params"]' in flow,
        "the server-initialization cell must set prune_plan, plan_id and n_params")
    chk(flow.index("compute_plan(CFG") < flow.index("C.resolve_resume")
        and flow.index("compute_plan(CFG") < flow.index("load_clients("),
        "the pruning plan must exist before the resume gate and before the decode")
    chk("prune_plan.json" in flow, "the plan must be written next to the run's artifacts")
    ...
    chk("build_model=build_model" in flow and 'expect_params=CFG["n_params"]' in flow,
        "the verifier must rebuild the pruned global model at the pruned parameter count")
    ...
    chk(re.fullmatch(rf"lwfednids_{K}c(_probe|_[a-z0-9]+)?", name) is not None,
        f"run_name {name!r} is not lwfednids_{K}c with an optional tag")

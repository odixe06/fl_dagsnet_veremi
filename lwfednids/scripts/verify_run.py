#!/usr/bin/env python
"""Verify a downloaded run directory offline: the global model's metrics == confusion ==
weights == history CSV, client stats == clients CSV, predictions rebuild the matrix, logs close,
and every weights file rebuilds the pruned model from its own cfg.
...
ok, lines = verify_run(a.run_dir, cfg=cfg,
                       ...
                       expect_params=None if a.no_rebuild else (cfg or {}).get("n_params"),

...
cache + content_id, the server's pruning step, the calibration micro-benchmark, training,
and the verifier cell. The pip cell is executed as written (torch-pruning is already in the
env, so pip is a no-op) — nothing about the flow is stubbed."""
...
NB = ROOT / "papers/lwfednids-bouayad-2024/notebook/20c_probe/lwfednids_20c_probe.ipynb"
CIDS = [0, 1, 2, 3]
...
def main():
    ...
    for i, src in enumerate(cells):
        ...
        if "compute_plan(CFG" in src:
            exec(src, ns)
            assert ns["CFG"]["n_params"] == 35_891 and ns["CFG"]["prune_plan"], "plan not set"
            print(f"-- cell {i}: server pruning step: {ns['CFG']['n_params']:,} params, plan {ns['CFG']['plan_id']}")
            continue
    ...
    if gpu:
        ...
        assert cal["pruned_train_backend"] == "eager" and "pruned_eval_eager_folded_16384" in cal
        assert "unpruned_train_eager_ms_per_step" in cal and "TA_train_eager_ms_per_step" in cal
        ...
        for f in ("history.csv", "clients.csv", "reports/manifest.json", "reports/y_true.u8.npy",
                  "reports/prune_plan.json", "weights/round_002.pt", "confusion/round_002.npy",
                  "logs/round_002.json", "complete/round_002.done"):
            assert (d / f).exists(), f
        mf = json.loads((d / "reports/manifest.json").read_text())
        assert mf["n_params"] == 35_891 and mf["prune"]["plan_id"] == mf["plan_id"] == ns["CFG"]["plan_id"]
    shutil.rmtree(tmp)
    print(f"\nNOTEBOOK SIMULATION PASSED ({'GPU: all cells up to and incl. calibration' if gpu else 'CPU: all cells but calibration, 2 rounds + verify'})")

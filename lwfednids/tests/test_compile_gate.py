...
Runs on the PRUNED model, the one the run actually compiles."""
import sys
...
CFGD = {**CFG, "n_features": 66, "clip": 1.0, "device": "cpu", "compile": True,
        "eval_batch": 64, "seed": 42, "sparsity": 0.7}
plan, info = compute_plan(CFGD)
CFGD.update(prune_plan=plan, plan_id=info["plan_id"], n_params=info["n_params"])
B = 64
...
def gate(wrapper, dropout=0.1):
    ...
    try:
        Sc = driver._compile_train(Se, cfg, torch.device("cpu"), xb, yb)
    finally:
        torch.compile = real
    ps = [m.p for m in Se.modules() if isinstance(m, torch.nn.Dropout)]
    trainable = Se.training and all(p.requires_grad for p in Se.parameters())
    return Sc is not Se, ps, trainable
...
def main():
    ...
    print(f"\nALL {n} COMPILE-GATE CHECKS PASSED (pruned model, {CFGD['n_params']:,} params)")
